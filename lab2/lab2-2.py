...
#ZADANIE NA OCENE 3.5 (prostokat), 4.0 (dodatkowe cechy prostokata), 4.5 (dywan sierpinskiego)
...
def render(time):
    ...
    if(len(sys.argv) != 2):
        print('usage:', __file__, '<carpet_level>')
        sys.exit(2)
...
def update_viewport(window, width, height):
    ...
    #Seed odpowiadający za kolory jest losowany za każdym razem przy zmianie rozmiaru okna
    global rand_seed
    rand_seed = time.time()
